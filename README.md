...
- длинная сторона **≤ 1600px**, JPEG **q80** (mozjpeg), EXIF срезается (ориентация запекается);
- фото уже меньшего размера (напр. из Telegram, 1280px) кладутся как есть;
- файлы **не заменяются по месту** — только новые папки `NN` и новые номера. На `/photos/subbotnik/` стоит месячный Cache-Control (см. `next.config.mjs`), замену по месту вернувшиеся посетители не увидят;
- новый альбом регистрируется в `Landscape.jsx` строкой `buildGroup('<год>', '<NN>', 'Название', <кол-во фото>)`.

Галереи глав Истории — 1920px, портреты команды — 800px.
